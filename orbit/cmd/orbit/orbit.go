...
func orbitAction(c *cli.Context) error {
	...
	// >>> OPENFRAME(agent-openframe-mode): skip the Fleet Desktop device-token check + rotation in
	// openframe mode — openframe/docs/agent-openframe-mode.md
	//
	// The device-token-authenticated path (/api/.../fleet/device/{token}/...) is NOT routed through
	// the OpenFrame gateway (device auth terminates via /tools/agent/fleetmdm-server with the bearer
	// token instead). So CheckToken always lands on the user-gateway's user-session route and 401s,
	// and Rotate() then churns the token every 5 min forever. Upstream
	// historically ran this only under --fleet-desktop, then ungated it for refetch-host/device-auth;
	// OpenFrame has no device-token consumer (My Device, fleetd error reporting, and ADE Setup
	// Experience are all off), so the loop is dead weight here. The token is still generated by
	// LoadOrGenerate above, so any reader still gets a value.
	if !c.Bool("openframe-mode") {
