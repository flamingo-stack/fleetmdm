...
func TestUp_20250904091745(t *testing.T) {
	db := applyUpToPrev(t)
	...
	customSCEPProxyCAs := []fleet.CustomSCEPProxyCA{{
...
FROM certificate_authorities`
	...
	for _, ca := range cas {
		...
		case "digicert":
			...
			require.NotNil(t, ca.CertificateUserPrincipalNames)
