...
| `orbit/cmd/orbit/orbit.go` | 4 `openframe-*` flags, custom osqueryd path, token-refresher startup, `uuid` cmd, osquery flag passthrough, `NewOrbitClient(..., openFrameMode, authManager)` args; `agent-skip-setup-experience`: openframe mode disables the setup experience (preflight + darwin receiver) and `processSetupExperience` returns before registering the poller when `init` said not enabled | Keep all; re-thread the two extra `NewOrbitClient` args at both call sites; keep the not-enabled early return ahead of `NewLinuxSetupExperiencer` |
| `orbit/pkg/setup_experience/setup_experience.go` | `agent-skip-setup-experience`: `LinuxSetupExperiencer.Run` returns early on `!info.Enabled` | Re-insert the guard right after the `info == nil \|\| info.TimeFinished != nil` check |
